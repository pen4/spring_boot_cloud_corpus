...
public class HBaseService {
     ...
     * @throws IOException io异常
     */
    public Map<String, String> scanLatestOneOfTable(String tableName, String family, String column, String value, Long startMillis, Long endMillis) throws IOException {
        Table table = connection.getTable(TableName.valueOf(tableName));
        ...
        if (StringUtils.isNotBlank(column)) {
            ValueFilter valueFilter = new ValueFilter(CompareOperator.EQUAL, new BinaryComparator(Bytes.toBytes(value)));
        ...
        try {
            for (Result result : scanner) {
                for (Cell cell : result.rawCells()) {
                    ...
                    System.out.println("row="+row+",values="+values+",qualifier="+qualifier);
     ...
     * @throws IOException
     */
    public Map<String, String> regexStringFilterQuery(String tableName, String cf, String column, String expr) throws IOException {
        Table table = connection.getTable(TableName.valueOf(tableName));
        Scan scan = new Scan();
        RegexStringComparator comp = new RegexStringComparator("my.");
        // any value that starts with 'my'
        SingleColumnValueFilter filter = new SingleColumnValueFilter(Bytes.toBytes(cf), Bytes.toBytes(column), CompareOperator.EQUAL, comp);
        scan.setFilter(filter);
        return getStringStringMap(table, scan);
    }
     ...
     * @throws IOException
     */
    public Map<String, String> columnRangeFilterQuery(String tableName, String cf,String start,String end) throws IOException {
        Table t = connection.getTable(TableName.valueOf(tableName));
        //  byte[] row =Row
        byte[] family = Bytes.toBytes(cf);
        byte[] startColumn = Bytes.toBytes(start);
        byte[] endColumn = Bytes.toBytes(end);
        Scan scan = new Scan(); // (optional) limit to one row
     ...
     * @param tableName 表
     * @param family 列族
     * @param startRow "21082022-id1-0"
     * @param endRow "29082022-id1-46080"
     * @return
     * @throws IOException
     */
    public Map<String, String> scanTableStartEndRow(String tableName, String family,  String startRow, String endRow) throws IOException {
        Table table = connection.getTable(TableName.valueOf(tableName));
        Scan scan = new Scan();
        scan.addFamily(Bytes.toBytes(family));
        scan.withStartRow(Bytes.toBytes(startRow)).withStopRow(Bytes.toBytes(endRow));
       /* FilterList filterList=new FilterList();
        RowFilter rowFilter=new RowFilter(CompareOperator.GREATER_OR_EQUAL,new SubstringComparator(startRow));
        RowFilter rowFilter2=new RowFilter(CompareOperator.LESS,new SubstringComparator(endRow));
