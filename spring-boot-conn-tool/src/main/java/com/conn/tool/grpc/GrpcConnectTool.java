 ...
 */
@Slf4j
public class GrpcConnectTool {

    public static void main(String[] args) throws InterruptedException {
        // 反射方法的格式只支持 package.service.method 或者 package.service
        String methodSymbol = "helloworld.Greeter.SayHello";
        String requestContent = "{\"name\": \"abcdedf\"}";
        String host = "127.0.0.1";
        int port = 50051;

        CodeMsg msg = execute(new RpcCallParams(methodSymbol, requestContent, host, port));
        if (msg.getCode() < 0) {
            log.error("执行调用grpc失败");
     ...
     * fanhua 调用入口方法
     *
     * @param rpcCallParams@return
     * @throws InterruptedException
     */
    public static CodeMsg execute(RpcCallParams rpcCallParams) throws InterruptedException {
        // 构建 Channel
        ManagedChannel channel = ManagedChannelBuilder.forAddress(rpcCallParams.getHost(), rpcCallParams.getPort()).usePlaintext().build();
        ...
        // 请求观察器
        StreamObserver<ServerReflectionRequest> requestStreamObserver = reflectionStub.serverReflectionInfo(getStreamObserver(rpcCallParams.getMethodSymbol(), rpcCallParams.getRequestContent(), channel, code));
        // 构建并发送获取方法文件描述请求
        ServerReflectionRequest getFileContainingSymbolRequest = ServerReflectionRequest.newBuilder().setFileContainingSymbol(rpcCallParams.getMethodSymbol()).build();
    ...
    private static StreamObserver<ServerReflectionResponse> getStreamObserver(String methodSymbol, String requestContent, ManagedChannel channel, CodeMsg codeMsg) {
        // 响应观察器
    ...
    private static Descriptors.FileDescriptor getFileDescriptor(List<ByteString> fileDescriptorProtoList, String packageName, String serviceName) throws Exception {
        ...
        Map<String, DescriptorProtos.FileDescriptorProto> fileDescriptorProtoMap = fileDescriptorProtoList.stream().map(bs -> {
            try {
            ...
            return null;
        }).filter(Objects::nonNull).collect(Collectors.toMap(DescriptorProtos.FileDescriptorProto::getName, f -> f));
    ...
    private static Descriptors.FileDescriptor[] getDependencies(DescriptorProtos.FileDescriptorProto proto, Map<String, DescriptorProtos.FileDescriptorProto> finalDescriptorProtoMap) {
        return proto.getDependencyList().stream().map(finalDescriptorProtoMap::get).map(f -> toFileDescriptor(f, getDependencies(f, finalDescriptorProtoMap))).toArray(Descriptors.FileDescriptor[]::new);
    }
     ...
     * 执行方法调用
     */
    private static void executeCall(ManagedChannel channel, Descriptors.FileDescriptor fileDescriptor, Descriptors.MethodDescriptor originMethodDescriptor, String requestContent) throws Exception {
        ...
        // 将响应解析为 JSON 字符串
        JsonFormat.Printer printer = JsonFormat.printer().usingTypeRegistry(registry).includingDefaultValueFields();
    ...
    private static MethodDescriptor<DynamicMessage, DynamicMessage> generateMethodDescriptor(Descriptors.MethodDescriptor originMethodDescriptor) {
        ...
        // 请求和响应类型
        MethodDescriptor.Marshaller<DynamicMessage> inputTypeMarshaller = ProtoUtils.marshaller(DynamicMessage.newBuilder(originMethodDescriptor.getInputType()).buildPartial());
        MethodDescriptor.Marshaller<DynamicMessage> outputTypeMarshaller = ProtoUtils.marshaller(DynamicMessage.newBuilder(originMethodDescriptor.getOutputType()).buildPartial());

        // 生成方法描述, originMethodDescriptor 的 fullMethodName 不正确
        return MethodDescriptor.<DynamicMessage, DynamicMessage>newBuilder().setFullMethodName(fullMethodName).setRequestMarshaller(inputTypeMarshaller).setResponseMarshaller(outputTypeMarshaller)
                // 使用 UNKNOWN，自动修改
